...
MRESULT EXPENTRY MainDlgProc(HWND hwndDlg, ULONG msg, MPARAM mp1, MPARAM mp2)
{
 ...
 /* ##END Top of window procedure */
   ...
   case WM_INITDLG :
     ...
     pMainFormInfo=(PMAINFORMINFO) WinQueryWindowULong(hwndDlg, QWL_USER);
      ...
      if ( (hini = PrfOpenProfile(WinQueryAnchorBlock(hwndDlg) , INIFILE)) != NULLHANDLE )
         {
         PrfQueryProfileSize(hini, "makekeys", "WINPOS", &rSize);
         if ( rSize == (sizeof(pgmData)) )
            {
            PrfQueryProfileData(hini, "makekeys", "WINPOS", &pgmData, &rSize);
            }
         else
            {
            pgmData[0] = 50;
      ...
      WinSetWindowPos(hwndFrame, HWND_TOP, pgmData[0], pgmData[1], 0, 0, SWP_MOVE );


      WinSendDlgItemMsg(hwndDlg, 1006,
                        MLM_SETTEXTLIMIT,
                        MPFROMLONG(500), 0);
      /* ##END  */
      }
   ...
   case WM_DESTROY :
      ...
      pgmData[0] = swp.x;
      pgmData[1] = swp.y;
      PrfWriteProfileData(hini, "makekeys", "WINPOS", &pgmData, sizeof(pgmData));
      /* ##END  */
      }
     HandleMessage(hwndFrame, hwndDlg, msg, mp1, mp2);
      break;
   ...
   case WM_COMMAND :
      ...
      case 1005:
         {
         /* ##START 1005.0  */
         /* Event Clicked/selected - ~Cancel  1005 */
